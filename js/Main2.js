	...
	$(".SubActivityDelete").live("click", function() {
		...
		var ActivityDelete = new Bmob.Query(ActivityDelete);
		var subactivityname = $(this).attr('SubActivityName');
		ActivityDelete.get($(this).attr("name"), {
			success: function(result) {
				swal({
					title: "Delete important stuff?",
					text: "Are you sure you want to delete " + subactivityname,
					type: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",
					confirmButtonText: "Yes, delete it！",
					...
					result.destroy({
						success: function(dres) {
							swal({
								...
								text: subactivityname + " has been deleted!",
								type: "success",
								confirmButtonText: "OK",
							}, function() {
								parent.location.reload();
	...
	$(".ActivityModify").live("click", function() {
		...
		$(".selectcolor").spectrum({
			...
			palette: [
				['#00FFFF', '#FF1493', '#0000FF', '#00BFFF', '#00FF00'],
				['#FF8C00', '#FF0000', '#9400D3', '#778899', '#980000'],
	...
	$("#ModifyActivitySave").click(function() {
		...
		query.get($(this).attr("activityid"), {
			success: function(results) {
				...
				swal({
					title: "Changes Saved！",
					text: $("input[id='ModifyActivityInput']").val() + " has been saved!",
					type: "success",
					confirmButtonText: "OK",
				}, function() {
	...
	$("#ModifySubActivitySave").click(function() {
		...
		query.get($(this).attr("Subactivityid"), {
			success: function(results) {
				...
				swal({
					title: "Changes Saved！",
					text: $("input[id='ModifySubActivityInput']").val() + " has been saved!",
					type: "success",
					confirmButtonText: "OK",
				}, function() {
					window.location.reload()
	...
	$("#SubActivitySave").click(function() {
		...
		post.save(null, {
			success: function(Post) {
				swal({
					title: "Sub-activity Saved！",
					text: $("input[id='SubActivityInput']").val() + " has been saved!",
					type: "success",
					confirmButtonText: "OK",
				}, function() {
					window.location.reload()
	...
	function statistics() {
		...
		var query = new Bmob.Query(Timetable1);
		query.equalTo("user", user);
		query.equalTo("Date", $("#datepicker2").val());
		query.find({
			success: function(results) {
				for(var i = 0; i < results.length; i++) {
					var object = results[i];
				...
				var name1 = [];
				for(var i = 0; i < 12; i++) {
					if(typeof(object.get("timetable" + i)) != 'undefined') {
						name1.push(object.get("timetable" + i));
					}
				}　
				//delete repetition
				console.log(name1);
				...
				for(var i = 0; i < name1.length; i++) {
					if(uniarr.indexOf(name1[i]) == -1) {
						uniarr.push(name1[i]);
					}
				}
				console.log(uniarr);

				// count activity/subactivity
				var countarr = Array();
				for(var i = 0; i < name1.length;) {
					var count = 0;
				...
				for(var i = 0; i < countarr.length; i++) {　　
					frequency.push(countarr[i].frequency);　　
				}
				...
				for(var i = 0; i < uniarr.length; i++) {
					(function(j) {
						...
						query.get(uniarr[j], {
							success: function(ActivityQuery) {
								actcolors[j] = ActivityQuery.get("ActivityColor");
								actnames[j] = ActivityQuery.get("ActivityName");
							},
							error: function() {
								var SubActivityQuery = Bmob.Object.extend("SubActivity");
								var query2 = new Bmob.Query(SubActivityQuery);
								query2.get(uniarr[j], {
									success: function(SubActivityQuery) {
										actcolors[j] = SubActivityQuery.get("SubActivityColor");
										actnames[j] = SubActivityQuery.get("SubActivityName");
									},
								});
							}
						});
